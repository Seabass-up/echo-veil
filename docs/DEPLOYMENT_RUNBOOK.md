...
4. Acquire the Azure guest-attestation JWT from the CVM. Confirm
   `sevsnpvm`, `azure-compliant-cvm`, `is-debuggable=false`, Secure Boot, vTPM,
   and the expected launch measurement.
...
9. Set `ECHO_VEIL_MEASUREMENT` to the reviewed immutable container-image digest,
   pin `ECHO_VEIL_IMAGE` by that digest, and make the CKKS directory owned by UID
   10001 with mode `0700`.
10. Install Caddy with `deploy/enclave/Caddyfile`, install the mTLS CA, and start
...
key and approved launch/image measurement, and
